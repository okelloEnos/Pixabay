...
class TrendingPhotoBloc extends Bloc<TrendingPhotoEvent, TrendingPhotoState> {
  ...
  TrendingPhotoBloc({required FetchPhotosUseCase useCase}) : _useCase = useCase, super(TrendingPhotoInitial()) {
  ...
  Future<void> _onFetchTrendingPhotos(FetchTrendingPhotos event, Emitter<TrendingPhotoState> emit) async {
    ...
    try {
      List<PhotoEntity> photos = await _useCase.call(query: 'popular', page: pageNo, perPage: perPage);
      allPhotos = [
        ...allPhotos,
        ...photos
      ];
      ...
      emit(TrendingPhotoLoaded(photos: allPhotos, hasReachedMax: hasReachedMax));
  ...
  void _onRefreshTrendingPhotos(RefreshTrendingPhotos event, Emitter<TrendingPhotoState> emit){
    pageNo = 1;
    allPhotos = [];
